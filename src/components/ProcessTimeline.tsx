...
const ProcessTimeline = () => (
  <section className="py-20 bg-gradient-to-br from-gray-50 to-white"><div className="container mx-auto px-6">
    ...
    <div className="max-w-4xl mx-auto"><div className="grid sm:grid-cols-4 gap-6 relative"><div className="hidden sm:block absolute top-10 left-[12%] right-[12%] h-0.5 bg-gray-200"></div>
      {steps.map((s, i) => <div key={i} className="relative text-center"><div className={`${s.color} h-20 w-20 rounded-2xl mx-auto mb-4 flex items-center justify-center text-white shadow-lg relative z-10 hover:scale-110 transition-transform`}><s.icon className="h-9 w-9" /></div><h3 className="font-display text-sm font-bold mb-2">{s.title}</h3><p className="text-xs text-gray-500 leading-relaxed">{s.desc}</p></div>)}
    </div></div>
  </div></section>
);
